...
export type AiAgentQaReadiness = {
  currentState: 'not_ready';
  ...
  realCredentialAllowed: false;
  realCallAllowed: false;
  futureAiAgentQaRoutes: string[];
  futureAiAgentQaCriteria: string[];
  futureAiInboundQaCriteria: string[];
  futureAiOutboundQaCriteria: string[];
  futureCampaignScopeRules: string[];
  futureScorecardRules: string[];
  futureSupervisorReviewRules: string[];
  futureImprovementProposalRules: string[];
  futureRiskFlagRules: string[];
  futureRuntimeBoundaries: string[];
  prohibitedCurrentActions: string[];
  nextSteps: string[];
};
...
export type QaRbacAccessScopeReadiness = {
  currentState: 'not_ready';
  ...
  realCredentialAllowed: false;
  realCallAllowed: false;
  futureRbacScopeFields: string[];
  futureRoles: string[];
  futureQaAccessScopes: string[];
  futureRoleRules: string[];
  futureSuperAdminRules: string[];
  futureInternalAdminRules: string[];
  futureClientAdminRules: string[];
  futureSupervisorRules: string[];
  futureQaAnalystRules: string[];
  futureAiQaReviewerRules: string[];
  futureHumanQaReviewerRules: string[];
...
export type QaEvaluationWorkflowReadiness = {
  currentState: 'not_ready';
  ...
  realCredentialAllowed: false;
  realCallAllowed: false;
  futureWorkflowScopeFields: string[];
  futureQaWorkflowRoutes: string[];
  futureEvaluationLifecycleSteps: string[];
  futureCallSelectionRules: string[];
  futureTranscriptRecordingRules: string[];
  futureScorecardBindingRules: string[];
  futureAiSuggestedScoreRules: string[];
  futureSupervisorReviewRules: string[];
  futureFinalScoreRules: string[];
  futureCoachingRules: string[];
  futureCalibrationRules: string[];
  futureDisputeRules: string[];
  futureReportVisibilityRules: string[];
  futureRbacPiiRules: string[];
  futureApprovalVersioningRules: string[];
  futureAuditRules: string[];
  futureRuntimeBoundaries: string[];
  prohibitedCurrentActions: string[];
...
export function buildRouteReadinessReport(input: ReadinessInput): RouteReadinessReport {
  ...
  const aiAgentQaReadiness: AiAgentQaReadiness = {
    currentState: 'not_ready',
    aiAgentQaApproved: false,
    aiAgentQaMode: 'read_only_design',
    campaignScopedStatus: 'read_only_design',
    ...
    futureAiAgentQaCriteria: [
      ...
      'policy_compliance_adherence',
      'consent_dnc_handling',
      'pii_redaction_risk',
      'hallucination_risk',
      'refusal_quality',
      'escalation_handoff_quality',
      'intent_detection_quality',
      'customer_sentiment_handling',
      'answer_accuracy',
      'call_flow_completion',
      'objection_handling',
      'appointment_setting_quality',
      'sales_qualification_quality',
      'customer_service_resolution_quality',
      'healthcare_compliance_safe_response_quality',
      'ai_latency_response_pacing_review',
      'silence_interruption_handling',
      'call_outcome_consistency',
      'transcript_quality',
      'risk_flag_detection',
      'improvement_proposal_readiness',
    ],
    futureAiInboundQaCriteria: [
      'AI inbound QA must evaluate how the AI answers, discloses, identifies intent, handles consent and DNC, grounds answers, escalates, and completes the inbound call flow',
      'AI inbound QA must evaluate prompt adherence, knowledge base grounding, refusal quality, hallucination risk, sentiment handling, response pacing, silence handling, and handoff quality',
      'AI inbound QA must support different criteria by company, client, campaign, project, line of business, program type, route, language, and compliance scope',
      'AI inbound QA must not answer real inbound calls in this readiness phase',
    ],
    futureAiOutboundQaCriteria: [
      'AI outbound QA must evaluate campaign permission, opening disclosure, consent/DNC handling, objection handling, appointment setting, sales qualification, outcome consistency, and close',
      'AI outbound QA must evaluate prompt adherence, policy adherence, knowledge base grounding, hallucination risk, escalation quality, pacing, interruption handling, and compliance-safe responses',
      'AI outbound QA must support different criteria by company, client, campaign, project, line of business, program type, dialer context, route, language, and compliance scope',
      'AI outbound QA must not execute AI outbound calls in this readiness phase',
    ],
    futureCampaignScopeRules: [
      'AI Agent QA must be campaign-scoped',
      'AI Agent QA must be scoped by company/client/campaign/project/lineOfBusiness',
      'AI Agent QA must follow Campaign AI Agent & QA Scope Readiness',
      'AI Agent QA must not be global-only',
      'AI Agent QA findings must remain inside authorized client/campaign/project scope',
      'AI Agent QA must evaluate AI calls differently from Human Agent QA',
    ],
    futureScorecardRules: [
      'AI Agent QA scorecards must be scoped by client/campaign/project and line of business',
      ...
      'Scorecards must support weighted AI-specific criteria in a future implementation',
      'Scorecard changes must be versioned, approved, auditable, and rollback-capable in a future implementation',
      'Scorecards must not be hardcoded or global-only',
    ],
    futureSupervisorReviewRules: [
      'AI-generated QA scores must not become final scores automatically',
      'Supervisor/admin review must be required before final score in a future implementation',
      'Supervisor final score changes must be auditable and scoped',
      'Supervisor review must not update prompts, knowledge bases, policies, handoff rules, scoring rules, tool boundaries, or runtime automatically',
    ],
    futureImprovementProposalRules: [
      'AI Agent QA findings may identify improvement proposal candidates',
      'Improvement proposals may reference prompt, knowledge base, policy, handoff, scoring, tool boundary, scorecard, or compliance gaps',
      'Improvement proposals must require approval before any future change',
      'AI must not self-learn or automatically change prompts, knowledge bases, policies, handoff rules, scoring rules, or tool boundaries based on QA findings',
    ],
    futureRiskFlagRules: [
      'Risk flags may identify hallucination risk, compliance risk, PII risk, consent/DNC risk, unsafe healthcare response risk, escalation failure, and customer harm risk',
      'Risk flags must be scoped by client/campaign/project and line of business',
      'Risk flags must not automatically alter runtime behavior',
      'Risk flags must be auditable and reviewable in a future implementation',
    ],
    futureRuntimeBoundaries: [
      'AI Agent QA readiness must not create storage',
      'AI Agent QA readiness must not create endpoints',
      'AI Agent QA readiness must not create CRUD',
      'AI Agent QA readiness must not create migrations',
      'AI Agent QA readiness must not ingest transcripts',
      'AI Agent QA readiness must not access recordings',
      'AI Agent QA readiness must not execute evaluation',
      'AI Agent QA readiness must not generate scores',
      'AI Agent QA readiness must not create final supervisor scores',
      'AI Agent QA readiness must not create coaching, calibration, reports, risk flags, scorecards, QA records, or improvement proposals',
      'AI Agent QA readiness must not connect OpenAI or open Realtime sessions',
      'AI Agent QA readiness must not execute tools, AI inbound calls, or AI outbound calls',
      'AI Agent QA readiness must not enable FastAGI, modify Asterisk/Vicidial, or change route behavior',
      'AI must not self-learn or change behavior autonomously based on AI Agent QA findings',
    ],
    prohibitedCurrentActions: [
      'Do not create AI Agent QA storage in this phase',
      'Do not create AI Agent QA endpoints in this phase',
      'Do not create AI Agent QA CRUD in this phase',
      'Do not create AI Agent QA migrations in this phase',
      'Do not create database tables in this phase',
      'Do not ingest transcripts in this phase',
      'Do not access recordings in this phase',
      'Do not create QA records in this phase',
      'Do not create scorecards in this phase',
      'Do not generate AI QA scores in this phase',
      'Do not create supervisor final scores in this phase',
      'Do not create coaching or calibration records in this phase',
      'Do not generate AI Agent QA reports in this phase',
      'Do not create improvement proposals in this phase',
      'Do not connect OpenAI',
      'Do not execute OpenAI API calls',
    ...
    nextSteps: [
      'Keep AI Agent QA readiness read-only, not ready, unapproved, storage-unimplemented, endpoint-unimplemented, and execution-blocked.',
      'Define future AI inbound and AI outbound QA scorecards separately from Human Agent QA scorecards.',
      'Define future campaign-scoped AI Agent QA storage, transcript, recording reference, evaluation, supervisor review, final score, coaching, calibration, report, risk flag, audit, redaction, versioning, and rollback contracts in separately approved phases.',
      ...
      'Require approved improvement proposal workflows before AI Agent QA findings can affect prompts, knowledge bases, policies, handoff rules, scoring rules, tool boundaries, scorecards, or runtime.',
    ],
  };
  ...
  const qaRbacAccessScopeReadiness: QaRbacAccessScopeReadiness = {
    currentState: 'not_ready',
    qaRbacAccessScopeApproved: false,
    qaRbacAccessScopeMode: 'read_only_design',
    serverSideRbacStatus: 'read_only_design',
    ...
    futureRbacScopeFields: [
      ...
      'scorecardScopeId',
      'promptKbPolicyScopeId',
      'handoffScoringToolBoundaryScopeId',
      'reportScopeId',
      'coachingScopeId',
      'calibrationScopeId',
      'auditScopeId',
    ...
    futureQaAccessScopes: [
      ...
      'Prompt/KB/policy scope',
      'Handoff/scoring/tool boundary scope',
      'Reports',
      'Coaching',
      'Calibration',
      'Audit',
    ...
    futureRoleRules: [
      ...
      'Access must support QA tracks and QA routes',
      'No role should get cross-client or cross-campaign access by default',
      'Scorecard access must be scoped',
      'Prompt/KB/policy access must be scoped',
      'Reports, coaching, calibration, and audit access must be scoped',
    ],
    futureSuperAdminRules: [
      'super_admin may have global visibility in a future implementation',
      'super_admin global visibility must still be auditable',
      'super_admin actions must support approval, versioning, rollback, and effective-date controls where required',
      'super_admin global visibility must not change runtime behavior automatically',
    ],
    futureInternalAdminRules: [
      'internal_admin must only manage assigned companies/clients/campaigns/projects',
      'internal_admin must not access unassigned scopes by default',
      'internal_admin access changes must be auditable in a future implementation',
      'internal_admin assignments must support approval, versioning, rollback, and effective-date controls in a future implementation',
    ],
    futureClientAdminRules: [
      'client_admin must only see assigned client/campaign scope',
      'client_admin must only manage assigned client/campaign scope',
      'client_admin must not access other clients or campaigns by default',
      'client_admin may only manage QA and AI Agent tools inside assigned scope in a future implementation',
      'client_admin raw PII access must default to denied unless future RBAC/redaction policy allows it',
    ],
    futureSupervisorRules: [
      'supervisor must only see assigned campaigns/teams/QA scopes',
      'supervisor must not access unassigned campaign or team scopes by default',
      'supervisor scorecard, coaching, calibration, and report access must be scoped',
      'supervisor raw PII access must default to denied unless future RBAC/redaction policy allows it',
    ],
    futureQaAnalystRules: [
      'qa_analyst must only see assigned QA review scopes',
      'qa_analyst must not access unassigned QA review scopes by default',
      'qa_analyst report, audit, scorecard, and calibration access must be scoped',
      'qa_analyst raw PII access must default to denied unless future RBAC/redaction policy allows it',
    ],
    futureAiQaReviewerRules: [
      'ai_qa_reviewer must only see assigned AI Agent QA scopes',
      'ai_qa_reviewer must only see assigned AI inbound QA and AI outbound QA scopes',
      'ai_qa_reviewer must not receive Human Agent QA access by default',
      'ai_qa_reviewer prompt, KB, policy, handoff, scoring, and tool boundary visibility must be scoped',
    ],
    futureHumanQaReviewerRules: [
      'human_qa_reviewer must only see assigned Human Agent QA scopes',
      'human_qa_reviewer must only see assigned Human inbound QA and Human outbound QA scopes',
      'human_qa_reviewer must not receive AI Agent QA access by default',
      'human_qa_reviewer scorecard, coaching, calibration, report, and audit visibility must be scoped',
    ],
    futurePiiRedactionRules: [
      'Access to raw PII must default to denied unless future RBAC and redaction policy explicitly allow it',
      'Redaction policy access must be scoped by company/client/campaign/project/lineOfBusiness',
      'Raw PII display must require future server-side authorization',
      'Raw PII access must be audited in a future implementation',
      'Browser-side filtering must not be trusted for PII protection',
    ],
    futureApprovalVersioningRules: [
      'Access changes must support approval in a future implementation',
      'Access changes must support versioning in a future implementation',
      'Access changes must support rollback in a future implementation',
      'Access changes must support effective-date controls in a future implementation',
      'Role assignments must identify actor, role, scope, approval status, version, reason, timestamp, and effective date in a future implementation',
      'Access changes must not automatically enable OpenAI, AI calls, FastAGI, Asterisk/Vicidial changes, or route behavior changes',
    ],
    futureAuditRules: [
      'Access changes must be auditable in a future implementation',
      'RBAC policy changes must be auditable in a future implementation',
      'Assignment changes must be auditable in a future implementation',
      'Permission changes must be auditable in a future implementation',
      'Audit events must include actor, role, timestamp, company/client/campaign/project scope, QA scope, action, before/after state, reason, approval, version, effective date, and correlation ID',
      'Audit records must not expose credentials or unauthorized raw PII',
      'This readiness phase must not create audit records',
    ],
    futureRuntimeBoundaries: [
      'QA RBAC / Access Scope readiness must not create RBAC storage',
      'QA RBAC / Access Scope readiness must not create role, permission, assignment, access grant, or audit storage',
      'QA RBAC / Access Scope readiness must not create CRUD, endpoints, migrations, users, roles, permissions, access grants, assignments, or audit records',
      'QA RBAC / Access Scope readiness must not change existing authentication',
      'QA RBAC / Access Scope readiness must not change existing authorization',
      'QA RBAC / Access Scope readiness must not change login/session behavior',
      'QA RBAC / Access Scope readiness must not create policy enforcement middleware',
      'QA RBAC / Access Scope readiness must not expose RBAC or assignment controls',
      'QA RBAC / Access Scope readiness must not connect OpenAI or execute OpenAI API calls',
      'QA RBAC / Access Scope readiness must not open Realtime sessions or expose agent tools',
      'QA RBAC / Access Scope readiness must not enable AI voice, AI inbound calls, AI outbound calls, FastAGI, Asterisk/Vicidial changes, or route behavior changes',
    ],
    prohibitedCurrentActions: [
      'Do not create RBAC storage',
      'Do not create role storage',
      'Do not create permission storage',
      'Do not create assignment storage',
      'Do not create access grant storage',
      'Do not create audit storage',
      'Do not create CRUD',
      'Do not create endpoints',
      ...
      'Do not create access grants',
      'Do not create client admin assignments',
      'Do not create supervisor assignments',
      'Do not create QA analyst assignments',
      'Do not create AI QA reviewer assignments',
      'Do not create Human QA reviewer assignments',
      'Do not change authentication',
      'Do not change authorization',
      'Do not change login/session behavior',
      'Do not create policy enforcement middleware',
      'Do not create audit records',
      'Do not expose RBAC controls',
      'Do not expose assignment controls',
      'Do not connect OpenAI',
      'Do not execute OpenAI API calls',
      'Do not open Realtime sessions',
      'Do not expose agent tools',
    ...
    nextSteps: [
      'Keep QA RBAC / Access Scope Readiness read-only, not ready, unapproved, storage-unimplemented, endpoint-unimplemented, CRUD-unimplemented, migration-unimplemented, OpenAI-disconnected, and execution-blocked.',
      'Define future server-side RBAC and scoped QA/AI Agent access contracts in a separately approved phase.',
      'Define future role, permission, assignment, access grant, approval, versioning, rollback, effective-date, audit, and redaction controls before any RBAC implementation.',
      'Keep users, roles, permissions, assignments, access grants, auth changes, authorization changes, login/session changes, policy enforcement middleware, audit writes, OpenAI connection, Realtime sessions, AI inbound, AI outbound, FastAGI, Asterisk/Vicidial changes, and route behavior changes blocked.',
      'Require future server-side scope checks for company/client/campaign/project/lineOfBusiness, QA tracks, QA routes, scorecards, prompts, KBs, reports, coaching, calibration, audit, and redaction policy access.',
    ],
  };

  const qaEvaluationWorkflowReadiness: QaEvaluationWorkflowReadiness = {
    currentState: 'not_ready',
    qaEvaluationWorkflowApproved: false,
    qaEvaluationWorkflowMode: 'read_only_design',
    aiAgentQaWorkflowStatus: 'read_only_design',
    ...
    futureQaWorkflowRoutes: [
      ...
      'Human Agent QA',
      'QA Center',
      'Campaign-scoped QA workflow',
    ],
    futureEvaluationLifecycleSteps: [
      'Select or sample a scoped call in a future implementation',
      'Reference transcript and recording metadata without direct playback controls in this readiness phase',
      'Bind the evaluation to an approved scorecard version in a future implementation',
      'Capture AI suggested score only as a non-final suggestion in a future implementation',
      'Require supervisor or manual human review before final score in a future implementation',
      'Expose coaching, calibration, dispute, report, and audit states as scoped future workflow artifacts',
    ],
    futureCallSelectionRules: [
      'Call selection must be scoped by company/client/campaign/project/lineOfBusiness',
      'Call sampling rules must not ingest calls in this readiness phase',
      'Call selection records must not be created in this readiness phase',
      'Call selection must follow future QA RBAC / Access Scope Readiness before any implementation',
    ],
    futureTranscriptRecordingRules: [
      'Transcript references must be scoped and redaction-aware in a future implementation',
      'Recording references must be metadata-only until recording access is explicitly approved in a future implementation',
      'This readiness phase must not ingest transcripts',
      'This readiness phase must not access recordings',
      'This readiness phase must not analyze audio',
      'Raw PII access must default to denied unless future RBAC/redaction policy allows it',
    ],
    futureScorecardBindingRules: [
      'QA evaluations must bind to an approved scorecard version in a future implementation',
      'Scorecard version binding must be scoped by company/client/campaign/project/lineOfBusiness and QA route',
      'Calibration must not modify scorecards automatically',
      'Scorecard changes must remain governed by QA Scorecard Configuration Readiness',
    ],
    futureAiSuggestedScoreRules: [
      'AI suggested scores must not be final scores automatically',
      'AI suggested scores must require future human review before final scoring',
      'AI suggested scores must not trigger OpenAI or AI calls in this readiness phase',
      'AI suggested score storage and execution are not implemented in this readiness phase',
    ],
    futureSupervisorReviewRules: [
      'Supervisor final score must require future human review/approval',
      'Supervisor/manual review must be scoped by company/client/campaign/project/lineOfBusiness and QA route',
      'Supervisor review must not create final scores in this readiness phase',
      'Supervisor review must follow future server-side RBAC and audit controls',
    ],
    futureFinalScoreRules: [
      'Final score creation must remain blocked in this readiness phase',
      'Final scores must not be created automatically from AI suggested scores',
      'Disputes must not change scores automatically without future approval',
      'Final score changes must be auditable and rollback-capable in a future implementation',
    ],
    futureCoachingRules: [
      'Coaching recommendations must not be automatically applied',
      'Coaching generation must remain blocked in this readiness phase',
      'Coaching visibility must be scoped by RBAC in a future implementation',
      'Coaching changes must be auditable in a future implementation',
    ],
    futureCalibrationRules: [
      'Calibration must not modify scorecards automatically',
      'Calibration execution must remain blocked in this readiness phase',
      'Calibration sessions must be scoped by company/client/campaign/project/lineOfBusiness and QA route in a future implementation',
      'Calibration changes must support approval, versioning, rollback, audit, and effective-date controls in a future implementation',
    ],
    futureDisputeRules: [
      'Disputes must not change scores automatically without future approval',
      'Dispute execution must remain blocked in this readiness phase',
      'Dispute workflows must be scoped by RBAC in a future implementation',
      'Dispute outcomes must be auditable and rollback-capable in a future implementation',
    ],
    futureReportVisibilityRules: [
      'Reports must be scoped by RBAC',
      'Report generation must remain blocked in this readiness phase',
      'Report visibility must respect company/client/campaign/project/lineOfBusiness scope in a future implementation',
      'Reports must not expose raw PII unless future RBAC/redaction policy allows it',
    ],
    futureRbacPiiRules: [
      'Server-side RBAC is required before QA evaluation workflow implementation',
      'Browser-side filtering alone is not sufficient',
      'Raw PII must default to denied unless future RBAC/redaction policy allows it',
      'QA workflow access must follow QA RBAC / Access Scope Readiness',
      'Reports, recordings, transcripts, scores, coaching, calibration, disputes, and audit visibility must be scoped',
    ],
    futureApprovalVersioningRules: [
      'Workflow changes must support approval in a future implementation',
      'Workflow changes must support versioning in a future implementation',
      'Workflow changes must support rollback in a future implementation',
      'Workflow changes must support effective-date controls in a future implementation',
      'Workflow changes must not automatically trigger OpenAI, AI calls, FastAGI, Asterisk/Vicidial changes, route behavior changes, or runtime behavior',
    ],
    futureAuditRules: [
      'Workflow changes must be auditable in a future implementation',
      'Evaluation lifecycle events must be auditable in a future implementation',
      'Score, coaching, calibration, dispute, report, transcript, recording, and redaction events must be auditable in a future implementation',
      'Audit events must include actor, timestamp, company/client/campaign/project scope, line of business, QA route, scorecard version, approval, version, effective date, before/after state, reason, and correlation ID',
      'This readiness phase must not create audit records',
    ],
    futureRuntimeBoundaries: [
      'QA Evaluation Workflow readiness must not create evaluation storage',
      'QA Evaluation Workflow readiness must not create QA records, call selection records, transcript records, recording access records, scores, coaching records, calibration records, dispute records, report records, or audit records',
      'QA Evaluation Workflow readiness must not create CRUD, endpoints, migrations, database tables, or storage',
      'QA Evaluation Workflow readiness must not execute QA evaluation or scoring',
      'QA Evaluation Workflow readiness must not ingest calls or transcripts',
      'QA Evaluation Workflow readiness must not access recordings or analyze audio',
      'QA Evaluation Workflow readiness must not generate coaching or reports',
      'QA Evaluation Workflow readiness must not change RBAC, auth, login, or session behavior',
      'QA Evaluation Workflow readiness must not connect OpenAI, execute OpenAI API calls, open Realtime sessions, expose agent tools, enable AI voice, enable AI inbound calls, enable AI outbound calls, enable FastAGI, modify Asterisk/Vicidial, or change route behavior',
    ],
    prohibitedCurrentActions: [
      'Do not create evaluation storage',
      'Do not create QA record storage',
      'Do not create call selection storage',
      'Do not create transcript storage',
      'Do not create recording access storage',
      'Do not create score storage',
      'Do not create coaching storage',
      'Do not create calibration storage',
      'Do not create dispute storage',
      'Do not create report storage',
      'Do not create audit storage',
      'Do not create CRUD',
      'Do not create endpoints',
      ...
      'Do not create QA records',
      'Do not create call selection records',
      'Do not create transcript records',
      'Do not create recording access records',
      'Do not create AI suggested scores',
      'Do not create supervisor final scores',
      'Do not create coaching records',
      'Do not create calibration records',
      ...
      'Do not ingest calls or transcripts',
      'Do not access recordings',
      'Do not analyze audio',
      'Do not generate coaching',
    ...
    nextSteps: [
      'Keep QA Evaluation Workflow Readiness read-only, not ready, unapproved, storage-unimplemented, endpoint-unimplemented, CRUD-unimplemented, migration-unimplemented, OpenAI-disconnected, and execution-blocked.',
      'Define future QA evaluation workflow contracts for AI Agent QA, Human Agent QA, ai_inbound, ai_outbound, human_inbound, and human_outbound in a separately approved phase.',
      'Define future call selection, sampling, transcript, recording, redaction, scorecard binding, AI suggested score, supervisor review, final score, coaching, calibration, dispute, report, RBAC, PII, approval, versioning, rollback, audit, and effective-date controls before implementation.',
      'Keep evaluations, QA records, scoring, call ingestion, transcript ingestion, recording access, audio analysis, final scores, coaching generation, calibration, disputes, report generation, audit writes, OpenAI connection, Realtime sessions, AI inbound, AI outbound, FastAGI, Asterisk/Vicidial changes, and route behavior changes blocked.',
      'Require future human review/approval before any AI suggested score can influence a final supervisor score.',
    ],
  };
  ...
  const checklist: ReadinessChecklistItem[] = [
    ...
    {
      id: 'ai-agent-qa-readiness-read-only',
      label: 'AI Agent QA readiness read-only',
      status: 'pass',
    ...
    {
      id: 'qa-rbac-access-scope-readiness-read-only',
      label: 'QA RBAC / Access Scope readiness read-only',
      status: 'pass',
      detail: 'QA RBAC / Access Scope readiness is read-only, not ready, not approved, server-side RBAC mapped, company/client/campaign/project/line-of-business scope mapped, QA reviewer scope mapped, raw PII denied by default, storage-unimplemented, CRUD-unimplemented, endpoint-unimplemented, RBAC enforcement-blocked, OpenAI-disconnected, execution-blocked, and exposes no users, roles, permissions, assignments, access grants, RBAC controls, OpenAI, runtime, call execution, FastAGI, Asterisk/Vicidial, or route controls.',
    },
    {
      id: 'qa-evaluation-workflow-readiness-read-only',
      label: 'QA Evaluation Workflow readiness read-only',
      status: 'pass',
