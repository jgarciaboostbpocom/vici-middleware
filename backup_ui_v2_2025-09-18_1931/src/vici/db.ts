...
export async function updateAccidCidForState(
  campaignId: string,
  stateCode: string,
  newDid: string
): Promise<{ ok: boolean; updated?: number; note?: string }> {
  const db = getPool();
  ...
  const [colRows] = await (await db).query(
    `SELECT COLUMN_NAME
  ...
  if (!col) return { ok:false, note:'No callerID column (cid_number/cid/cidnum/outbound_cid) found' };
  ...
  for (const r of rows as any[]) {
    ...
    if (abbr===want) targets.push(String(r.areacode));
  }
  if (!targets.length) return { ok:false, note:`No ACTIVE AC-CID rows matched state ${stateCode}` };
