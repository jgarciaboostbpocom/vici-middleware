...
const pilotDocPath = 'docs/campaign-pilot-readiness.md';
...
const pilotDoc = exists(pilotDocPath) ? read(pilotDocPath) : '';
...
const results = [
  check('readiness helper includes campaignPilotReadiness', readiness.includes('campaignPilotReadiness')),
  ...
  check('readiness response contains pilotMode: "read_only"', sourceContainsValue(readiness, 'pilotMode', 'read_only')),
  ...
  check('readiness response contains pilotBlockers', readiness.includes('pilotBlockers')),
  check('readiness response contains readinessItems', readiness.includes('readinessItems')),
  check('UI contains Campaign Pilot Readiness', ui.includes('Campaign Pilot Readiness')),
  check('UI contains Not ready', ui.includes('Not ready')),
  check('UI contains Read-only', ui.includes('Read-only')),
  check('UI contains Pilot blocked', ui.includes('Pilot blocked')),
  check('UI contains TESTCAMP candidate', ui.includes('TESTCAMP candidate')),
  ...
  check('campaign pilot readiness doc exists', exists(pilotDocPath)),
  check('campaign pilot readiness doc states read-only', /read-only/i.test(pilotDoc)),
  check('campaign pilot readiness doc states it does not approve a pilot', pilotDoc.includes('does not approve a pilot')),
  check('campaign pilot readiness doc states it does not enable live', pilotDoc.includes('does not enable live routing')),
  check('campaign pilot readiness doc states it does not touch Asterisk/Vicidial', pilotDoc.includes('does not touch Asterisk/Vicidial')),
  check(
    'middleware current status references campaign pilot readiness',
    statusDoc.includes('Campaign pilot readiness') &&
