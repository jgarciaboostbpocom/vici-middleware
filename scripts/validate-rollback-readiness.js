...
const rollbackDocPath = 'docs/rollback-readiness.md';
...
const results = [
  check('readiness helper includes rollbackReadiness', readiness.includes('rollbackReadiness')),
  check('readiness response contains currentState: "not_ready"', sourceContainsValue(readiness, 'currentState', 'not_ready')),
  check('readiness response contains rollbackApproved: false', readiness.includes('rollbackApproved: false')),
  check('readiness response contains rollbackMode: "read_only"', sourceContainsValue(readiness, 'rollbackMode', 'read_only')),
  check('readiness response contains operatorChecklistStatus', readiness.includes('operatorChecklistStatus')),
  check('readiness response contains fastAgiDisableProcedureStatus', readiness.includes('fastAgiDisableProcedureStatus')),
  check('readiness response contains routeEngineShadowProcedureStatus', readiness.includes('routeEngineShadowProcedureStatus')),
  check('readiness response contains fastAgiPortClosedVerificationStatus', readiness.includes('fastAgiPortClosedVerificationStatus')),
  check('readiness response contains asteriskDialplanRestoreProcedureStatus', readiness.includes('asteriskDialplanRestoreProcedureStatus')),
  check('readiness response contains callerIdDisableVerificationStatus', readiness.includes('callerIdDisableVerificationStatus')),
  check('readiness response contains serviceRestartApprovalStatus', readiness.includes('serviceRestartApprovalStatus')),
  check('readiness response contains emergencyContactStatus', readiness.includes('emergencyContactStatus')),
  ...
  check('readiness response contains rollbackBlockers', readiness.includes('rollbackBlockers')),
  ...
  check('UI contains Rollback Readiness', ui.includes('Rollback Readiness')),
  check('UI contains Not ready', ui.includes('Not ready')),
  check('UI contains Read-only', ui.includes('Read-only')),
  check('UI contains Rollback not approved', ui.includes('Rollback not approved')),
  check('UI contains Manual only', ui.includes('Manual only')),
  ...
  check('rollback readiness doc exists', exists(rollbackDocPath)),
  check('rollback readiness doc states read-only', /read-only/i.test(rollbackDoc)),
  check('rollback readiness doc states it does not execute rollback', rollbackDoc.includes('does not execute rollback')),
  check('rollback readiness doc states it does not restart services', rollbackDoc.includes('does not restart services')),
  check('rollback readiness doc states it does not touch Asterisk/Vicidial', rollbackDoc.includes('does not touch Asterisk/Vicidial')),
  check(
    'middleware current status references rollback readiness',
    statusDoc.includes('Rollback readiness') &&
      statusDoc.includes('rollbackApproved') &&
